define(["lodash","jquery","angular","moment","app/core/utils/kbn","app/core/core","app/plugins/sdk","app/core/utils/ticks","app/core/core_module","app/core/config","app/core/time_series2"], function(__WEBPACK_EXTERNAL_MODULE_0__, __WEBPACK_EXTERNAL_MODULE_1__, __WEBPACK_EXTERNAL_MODULE_2__, __WEBPACK_EXTERNAL_MODULE_4__, __WEBPACK_EXTERNAL_MODULE_5__, __WEBPACK_EXTERNAL_MODULE_6__, __WEBPACK_EXTERNAL_MODULE_8__, __WEBPACK_EXTERNAL_MODULE_22__, __WEBPACK_EXTERNAL_MODULE_31__, __WEBPACK_EXTERNAL_MODULE_33__, __WEBPACK_EXTERNAL_MODULE_35__) { return /******/ (function(modules) { // webpackBootstrap
/******/ 	// The module cache
...
(function ($) {
  ...
  function Plot(placeholder, data_, options_, plugins) {
    ...
    function setupTickGeneration(axis) {
      ...
      axis.scaledDecimals = axis.tickDecimals - Math.floor(Math.log(axis.tickSize) / Math.LN10); // Time mode was moved to a plug-in in 0.8, and since so many people use it
  ...
  return {
    ...
    link: function link(scope, elem) {
      ...
      function render_panel() {
        ...
        thresholdManager.prepare(elem, data); // un-check dashes if lines are unchecked

        panel.dashes = panel.lines ? panel.dashes : false;
        var stack = panel.stack ? true : null; // Populate element
        ...
        for (var i = 0; i < data.length; i++) {
          var series = data[i];
          series.data = series.getFlotPairs(series.nullPointMode || panel.nullPointMode); // if hidden remove points and disable stack
          ...
          case 'series':
            {
              ...
              for (var _i = 0; _i < data.length; _i++) {
                var _series = data[_i];
                _series.data = [[_i + 1, _series.stats[panel.xaxis.values[0]]]];
              }
      ...
      function addXHistogramAxis(options, bucketSize) {
        ...
        if (data.length && bucketSize) {
          ...
          while (ticks_num > defaultTicks) {
            _tickStep = _tickStep * 2;
            ticks_num = Math.ceil((max - min) / _tickStep);
          } // Expand ticks for pretty view


          min = Math.floor(min / _tickStep) * _tickStep;
          max = Math.ceil(max / _tickStep) * _tickStep;
          ticks = [];

          for (var i = min; i <= max; i += _tickStep) {
            ticks.push(i);
          }
        } else {
...
_core.coreModule.directive('grafanaGraph', graphDirective);
  ...
  function init(plot) {
    ...
    function onMouseDown(e) {
      if (e.which != 1) // only accept left-click
        return; // cancel out any text selections
      ...
      selection.active = true; // this is a bit silly, but we have to use a closure to be
    ...
    function onMouseUp(e) {
      mouseUpHandler = null; // revert drag stuff for old-school browsers

      if (document.onselectstart !== undefined) document.onselectstart = savedhandlers.onselectstart;
      if (document.ondrag !== undefined) document.ondrag = savedhandlers.ondrag; // no more dragging

      selection.active = false;
      updateSelection(e);
      if (selectionIsSane()) triggerSelectedEvent(e);else {
    ...
    function getSelection() {
      ...
      var r = {},
          c1 = selection.first,
          c2 = selection.second;
      var axes = plot.getAxes(); // look if no axis is used
    ...
    function triggerSelectedEvent(event) {
      var r = getSelection(); // Add ctrlKey and metaKey to event
...
(function ($) {
  ...
  function init(plot) {
    ...
    var stackBases = {};
    var processed = false;
    var stackSums = {}; //set percentage for stacked chart
    ...
    function stackData(plot, s, datapoints) {
      ...
      for (var i = 0; i < datapoints.points.length; i += 3) {
        ...
        stackBases[datapoints.points[i + key_idx]] += datapoints.points[i + value_idx]; // change points to percentage values
...
(function ($) {
  ...
  function init(plot) {
    ...
    function segmentIntersection(top_left_x, top_left_y, top_right_x, top_right_y, bottom_left_x, bottom_left_y, bottom_right_x, bottom_right_y) {
      ...
      s = (-top_delta_y * (top_left_x - bottom_left_x) + top_delta_x * (top_left_y - bottom_left_y)) / (-bottom_delta_x * top_delta_y + top_delta_x * bottom_delta_y);
      t = (bottom_delta_x * (top_left_y - bottom_left_y) - bottom_delta_y * (top_left_x - bottom_left_x)) / (-bottom_delta_x * top_delta_y + top_delta_x * bottom_delta_y); // Collision detected

      if (s >= 0 && s <= 1 && t >= 0 && t <= 1) {
        return [top_left_x + t * top_delta_x, // X
    ...
    function plotDifferenceArea(plot, ctx, series) {
      ...
      function validateInput() {
        ...
        for (i = 0; i < points.length / ps; i++) {
          if (points[i * ps] !== null && otherpoints[i * otherps] !== null && points[i * ps] !== otherpoints[i * otherps]) {
      ...
      function findNextStart(start_i, end_i) {
        ...
        var start = start_i === 0 || points[start_i - 1] === null || otherpoints[start_i - 1] === null,
            equal = false,
            i,
            intersect;
      ...
      function intersectionPoint(right_i) {
        ...
        for (i = 1; i < otherpoints.length / otherps; i++) {
          intersect = segmentIntersection(points[(right_i - 1) * ps], points[(right_i - 1) * ps + 1], points[right_i * ps], points[right_i * ps + 1], otherpoints[(i - 1) * otherps], otherpoints[(i - 1) * otherps + 1], otherpoints[i * otherps], otherpoints[i * otherps + 1]);
      ...
      function bottomTraversal(start_i, end_i) {
        ...
        for (i = start_i; i >= end_i; i--) {
          ctx.lineTo(otherseries.xaxis.p2c(otherpoints[i * otherps]) + plotOffset.left, otherseries.yaxis.p2c(otherpoints[i * otherps + 1]) + plotOffset.top);
        }
      ...
      function topTraversal(start_i, end_i) {
        ...
        for (i = start_i; i < end_i; i++) {
          if (points[i * ps + 1] === null && i > start_i) {
            ...
            intersect = intersectionPoint(i);
            ctx.lineTo(series.xaxis.p2c(intersect[0]) + plotOffset.left, series.yaxis.p2c(intersect[1]) + plotOffset.top);
            bottomTraversal(i, start_i);
            findNextStart(i, end_i);
            return;
          } else {
            ctx.lineTo(series.xaxis.p2c(points[i * ps]) + plotOffset.left, series.yaxis.p2c(points[i * ps + 1]) + plotOffset.top);
          }
        }
      ...
      fillStyle.a = 0.4;
      fillStyle.normalize();
      ctx.fillStyle = fillStyle.toString(); // Begin recursive bi-directional traversal
    ...
    plot.hooks.processDatapoints.push(function (plot, series, datapoints) {
      ...
      plot.hooks.draw.push(function (plot, ctx) {
        ...
        function plotDashes(xoffset, yoffset) {
          ...
          for (var i = ps; i < points.length; i += ps) {
            var lineSegmentOffset = function lineSegmentOffset(segmentLength) {
...
var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(1), __webpack_require__(0), __webpack_require__(2), __webpack_require__(20)], __WEBPACK_AMD_DEFINE_RESULT__ = (function ($, _, angular, Drop) {
  'use strict';
  ...
  var DrawableEvent = function DrawableEvent(object, drawFunc, clearFunc, moveFunc, left, top, width, height) {
    var _object = object;
  ...
  var VisualEvent = function VisualEvent(options, drawableEvent) {
    var _parent;
  ...
  var EventMarkers = function EventMarkers(plot) {
    var _events = [];
    this._types = [];
    this._plot = plot;
    ...
    this.drawEvents = function () {
      var that = this; // var o = this._plot.getPlotOffset();
    ...
    this._buildDiv = function (event) {
      ...
      var markerTooltip; // determine the y axis used
    ...
    this._buildRegDiv = function (event) {
      ...
      var yaxis, top, left, lineWidth, regionWidth, lineStyle, color, markerTooltip; // determine the y axis used
...
var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(1), __webpack_require__(6)], __WEBPACK_AMD_DEFINE_RESULT__ = (function ($, core) {
  'use strict';
  ...
  function GraphTooltip(elem, dashboard, scope, getSeriesFn) {
    ...
    this.getMultiSeriesPlotHoverInfo = function (seriesList, pos) {
      var value, i, series, hoverIndex, hoverDistance, pointTime, yaxis; // 3 sub-arrays, 1st for hidden series, 2nd for left yaxis, 3rd for right yaxis.

      var results = [[], [], []]; //now we know the current X (j) position for X and Y values
    ...
    this.show = function (pos, item) {
      ...
      var group, value, absoluteTime, hoverInfo, i, series, seriesHtml, tooltipFormat; // if panelRelY is defined another panel wants us to show a tooltip
      // get pageX from position on x axis and pageY from relative position in original panel

      if (pos.panelRelY) {
        var pointOffset = plot.pointOffset({
          x: pos.x
        });
...
function () {
  ...
  _createClass(ThresholdManager, [{
    key: "getHandleHtml",
    value: function getHandleHtml(handleIndex, model, valueStr) {
      ...
      return "\n    <div class=\"alert-handle-wrapper alert-handle-wrapper--T".concat(handleIndex, "\">\n      <div class=\"alert-handle-line alert-handle-line--").concat(stateClass, "\">\n      </div>\n      <div class=\"alert-handle\" data-handle-index=\"").concat(handleIndex, "\">\n        <i class=\"icon-gf icon-gf-").concat(stateClass, " alert-state-").concat(stateClass, "\"></i>\n        <span class=\"alert-handle-value\">").concat(valueStr, "<i class=\"alert-handle-grip\"></i></span>\n      </div>\n    </div>");
    }
  }, {
    key: "initDragging",
    value: function initDragging(evt) {
      ...
      function stopped() {
        isMoving = false; // calculate graph level

        var graphValue = plot.c2p({
          left: 0,
          top: posTop
        }).y;
        ...
        handleElem.off("mouseleave", dragging); // trigger digest and render
    ...
    key: "renderHandle",
    value: function renderHandle(handleIndex, defaultHandleTopPos) {
      var model = this.thresholds[handleIndex];
      ...
      if (!_lodash.default.isNumber(value)) {
        ...
        var valueCanvasPos = this.plot.p2c({
          x: 0,
          y: value
        });
    ...
    key: "shouldDrawHandles",
    value: function shouldDrawHandles() {
      return !this.hasSecondYAxis && this.panelCtrl.editingThresholds && this.panelCtrl.panel.thresholds.length > 0;
    }
  }, {
    key: "prepare",
    value: function prepare(elem, data) {
    ...
    key: "addFlotOptions",
    value: function addFlotOptions(options, panel) {
...
function () {
  ...
  _createClass(EventManager, [{
    ...
    key: "editorOpened",
    value: function editorOpened() {
      this.editorOpen = true;
    }
  }, {
    key: "updateTime",
    value: function updateTime(range) {
      if (!this.event) {
    ...
    key: "editEvent",
    value: function editEvent(event, elem) {
      this.event = event;
      this.panelCtrl.render();
    }
  }, {
    ...
    value: function addFlotEvents(annotations, flotOptions) {
      ...
      if (this.event) {
        ...
        for (var i = 0; i < annotations.length; i++) {
          var item = annotations[i]; // add properties used by jquery flot events
...
/* WEBPACK VAR INJECTION */(function(module) {var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(2), __webpack_require__(0), __webpack_require__(1)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (angular, _, $) {
  'use strict';
    ...
    return {
      link: function link(scope, elem) {
        ...
        function render() {
          ...
          seriesList = data;
          $container.empty(); // Set min-width if side style and there is a value, otherwise remove the CSS propery
...
/* WEBPACK VAR INJECTION */(function(module) {var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(2), __webpack_require__(1), __webpack_require__(0)], __WEBPACK_AMD_DEFINE_RESULT__ = (function (angular, jquery, _) {
  'use strict';
  ...
  module.controller('SeriesOverridesCtrl', function ($scope, $element, popoverSrv) {
    ...
    $scope.setOverride = function (item, subItem) {
      ...
      $scope.override[item.propertyName] = subItem.value; // automatically disable lines for this series and the fill bellow to series
...
function () {
  ...
  _createClass(ThresholdFormCtrl, [{
    ...
    key: "onFillColorChange",
    value: function onFillColorChange(index) {
      var _this2 = this;

      return function (newColor) {
        _this2.panel.thresholds[index].fillColor = newColor;

        _this2.render();
      };
    }
  }, {
    key: "onLineColorChange",
    value: function onLineColorChange(index) {
      var _this3 = this;

      return function (newColor) {
        _this3.panel.thresholds[index].lineColor = newColor;
...
_core_module.default.directive('graphThresholdForm', function () {
  return {
    restrict: 'E',
    template: template,
...
function () {
  ...
  _createClass(AxesEditorCtrl, [{
    ...
    key: "xAxisModeChanged",
    value: function xAxisModeChanged() {
      this.panelCtrl.processor.setPanelDefaultsForNewXAxisMode();
      this.panelCtrl.onDataReceived(this.panelCtrl.dataList);
    }
  }, {
    key: "xAxisValueChanged",
    value: function xAxisValueChanged() {
      this.panelCtrl.onDataReceived(this.panelCtrl.dataList);
    }
  }, {
    key: "getDataFieldNames",
